...
public class SMAConditionChecker extends ConditionChecker {
    ...
    public boolean checkCondition(Condition condition, PriceIndicator priceIndicator) {
        ...
        if (condition.indicator2.equals(IndicatorType.price)) {
            ...
            return compareNumbers(condition.operator, smaValue, lastValue, condition.diff_percent);
            ...
            return compareNumbers(condition.operator, emaValue, emaValue, condition.diff_percent);
        } else if (condition.indicator2.equals(IndicatorType.week52High)) {
            return compareNumbers(condition.operator, smaValue, stock.week52High, condition.diff_percent);
        } else if (condition.indicator2.equals(IndicatorType.week52Low)) {
            return compareNumbers(condition.operator, smaValue, stock.week52Low, condition.diff_percent);
        } else {
            return false;
        }
    }
